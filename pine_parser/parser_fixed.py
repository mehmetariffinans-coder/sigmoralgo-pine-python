...
class Parser:
    ...
    def parse_postfix(self) -> ast.ASTNode:
        ...
        FIX: Doğru sıra: primary -> assignment -> postfix operatörler
        """
        # ✅ FIX: Önce primary expression parse et
        ...
        while True:
            ...
            # ✅ FIX: Artık expression herhangi bir type olabilir (Identifier, MethodCall, vb.)
            elif self.match(TokenType.LPAREN) and isinstance(expr, ast.Identifier):
                # Function call - only for bare identifiers
                self.advance()
                args = self.parse_arguments()
                self.expect(TokenType.RPAREN)
        ...
        # ✅ FIX: Postfix operatörleri uyguladıktan sonra assignment kontrol et
        return self.parse_assignment_postfix(expr)
    
    def parse_assignment_postfix(self, expr: ast.ASTNode) -> ast.ASTNode:
        """Parse assignment after postfix operators
        
        Bu fonksiyon parse_postfix tarafından çağrılır
        """
