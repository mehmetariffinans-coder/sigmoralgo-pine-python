...
class CodeGenerator:
    ...
    def __init__(self):
        ...
        self.indent_level = 0
        self.imports: Set[str] = {'pandas as pd', 'pandas_ta as ta', 'backtrader as bt'}
    ...
    def _init_indicator_mapping(self) -> Dict[str, str]:
        ...
        return {
            'ta.sma': 'bt.indicators.SimpleMovingAverage',
            'ta.ema': 'bt.indicators.ExponentialMovingAverage',
            'ta.rsi': 'bt.indicators.RSI',
            'ta.macd': 'bt.indicators.MACD',
            'ta.bb': 'bt.indicators.BollingerBands',
            'ta.atr': 'bt.indicators.AverageTrueRange',
            'ta.stoch': 'bt.indicators.Stochastic',
        }
    ...
    def _emit_imports(self):
        ...
        self._emit_line("import pandas as pd")
        self._emit_line("import pandas_ta as ta")
        self._emit_line("import backtrader as bt")
        self._emit_line("import numpy as np")
    ...
    def _generate_assignment(self, node: ast.Assignment) -> str:
        ...
        if node.operator == '=':
            self._emit_line(f"{node.target} = {value}")
        elif node.operator == '+=':
            self._emit_line(f"{node.target} += {value}")
        elif node.operator == '-=':
            self._emit_line(f"{node.target} -= {value}")
    ...
    def _generate_unary_op(self, node: ast.UnaryOp) -> str:
        ...
        if node.operator in ['-', 'not', '!']:
            return f"(-{operand})" if node.operator == '-' else f"(not {operand})"
    ...
    def _generate_function_call(self, node: ast.FunctionCall) -> str:
        ...
        elif node.name == 'request.security':
            # Convert request.security to simpler form
            return f"df[{args.split(',')[-1].strip()}]"
